...
$settings = (Get-Content ((Get-ChildItem -Path $buildProjectFolder -Filter "build-build-settings.json" -Recurse).FullName) -Encoding UTF8 | Out-String | ConvertFrom-Json)
